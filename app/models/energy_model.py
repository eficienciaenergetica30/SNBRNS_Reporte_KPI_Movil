...
def get_energy_data(costcenter, date, sitename=''):
    """
    ...
    try:
        ...
        if normalized_sitename:
            pf_query = f'''
                ...
                FROM
                    (SELECT "HOUR", TOTALPOWERFACTOR FROM "{schema}"."{view}" WHERE "COSTCENTER" = ? AND "DATE" = ? AND "SITE_NAME" = ? AND TOTALPOWERFACTOR IS NOT NULL ORDER BY TOTALPOWERFACTOR ASC LIMIT 1) AS MIN_Q
                CROSS JOIN
                    (SELECT "HOUR", TOTALPOWERFACTOR FROM "{schema}"."{view}" WHERE "COSTCENTER" = ? AND "DATE" = ? AND "SITE_NAME" = ? AND TOTALPOWERFACTOR IS NOT NULL ORDER BY TOTALPOWERFACTOR DESC LIMIT 1) AS MAX_Q
                CROSS JOIN
                    (SELECT "HOUR", TOTALPOWERFACTOR FROM "{schema}"."{view}" WHERE "COSTCENTER" = ? AND "DATE" = ? AND "SITE_NAME" = ? AND TOTALPOWERFACTOR IS NOT NULL ORDER BY "HOUR" DESC LIMIT 1) AS CURRENT_Q
                CROSS JOIN
                    (SELECT SUM(TOTALPOWERFACTOR) AS TOTAL_POWER_FACTOR, COUNT(*) AS CNT FROM "{schema}"."{view}" WHERE "COSTCENTER" = ? AND "DATE" = ? AND "SITE_NAME" = ? AND TOTALPOWERFACTOR IS NOT NULL GROUP BY "DATE") AS TOTAL_Q
            '''
            cursor.execute(
                pf_query,
                (
                    costcenter, date, normalized_sitename,
                    costcenter, date, normalized_sitename,
                    costcenter, date, normalized_sitename,
                    costcenter, date, normalized_sitename,
                )
...
def get_sites():
    """
    Trae ÚNICAMENTE los sitios que tienen datos en la vista de Energía.
    ...
    try:
        ...
        print(f"Error al obtener sitios energía: {e}")
        return None
